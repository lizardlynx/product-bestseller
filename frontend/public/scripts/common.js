...
export function createPagination(pageCount, pages, pageNumber, url) {
  pages.innerHTML += `<a class="page ${
    pageNumber - 1 == 0 ? 'disabled' : ''
  }" href="${url}?pageNumber=${pageNumber - 1}"><</a>`;
  ...
  if (pageCount > 1) {
    ...
    for (let i = 1; i < pageCount - 1; i++) {
      if (
        i >= pageNumber - 1 - leftVisiblePages &&
        i <= pageNumber - 1 + rightVisiblePages
      )
        pages.innerHTML += `<a class="page ${
          i + 1 == pageNumber ? 'disabled' : ''
        }" href="${url}?pageNumber=${i + 1}">${i + 1}</a>`;
      else if (
        i == pageNumber - 2 - leftVisiblePages ||
        i == pageNumber + rightVisiblePages
      )
        pages.innerHTML += '...';
    }
    pages.innerHTML += `<a class="page ${
      pageNumber == pageCount ? 'chosen' : ''
    }" href="${url}?pageNumber=${pageCount}">${pageCount}</a>`;
  }

  pages.innerHTML += `<a class="page ${
    pageNumber + 1 > pageCount ? 'disabled' : ''
  }" href="${url}?pageNumber=${pageNumber + 1}">></a>`;
}

export function openTab(e, postfix = '') {
  const tabs = document.getElementsByClassName('tab-holder' + postfix);
  const btns = document.getElementsByClassName('tab-opener-button' + postfix);
...
export function addSearchItem(item, id, additionalClasses, callback = null) {
  ...
  item.shops.forEach(
    (shop) =>
      (imgs += `<div class="shop-logo"><img src="/images/${dbShopsData[shop].title}.png"></div>`)
  );
  imgs += '</div>';
  itemA.innerHTML =
    imgs + `<div class="search-product-title">${item.title}</div>`;
...
export function initSearch(
  ...
  productSearch.addEventListener('keyup', (e) =>
    searchProduct(e, resultId, additionalClasses, callback)
  );
  productSearch.addEventListener('focusout', hideResultSet);
  productSearch.addEventListener('focusin', (e) =>
    showSearchResults(e, resultId)
  );

  const searchRes = document.getElementById(resultId);
  searchRes.addEventListener('mouseenter', () =>
    productSearch.removeEventListener('focusout', hideResultSet)
  );
...
function searchProduct(e, resultId, additionalClasses, callback) {
  ...
  if (e.target.value.trim().length == 0)
    return hideSearchResults(null, resultId);
  interval = setTimeout(async () => {
    ...
    resJSON.forEach((item) =>
      addSearchItem(item, resultId, additionalClasses, callback)
    );
  }, 1000);
}

export function updateTabOpenerListeners(postfix = '') {
  const tableButtons = document.getElementsByClassName(
    'tab-opener-button' + postfix
  );
  for (const btn of tableButtons) {
    btn.addEventListener('click', (e) => openTab(e, postfix));
  }
}

export function deleteExtraCache(prefix) {
