...
(function (factory) {
    ...
    function (BoostableMap, U) {
      ...
      function getBoostClipRect(chart, target) {
        ...
        if (target === chart) {
          ...
          if (verticalAxes.length <= 1) {
            ...
            clipBox.height =
              verticalAxes[0].pos - chart.plotTop + verticalAxes[0].len;
          } else {
      ...
      function isChartSeriesBoosting(chart) {
        ...
        for (const series of allSeries) {
          ...
          // See #9046
          if (seriesOptions.boostThreshold === 0 || series.visible === false) {
            continue;
          }
        ...
        boost.forceChartBoost =
          allowBoostForce &&
          ((canBoostCount === allSeries.length && needBoostCount > 0) ||
            needBoostCount > 5);
        return boost.forceChartBoost;
      }
      /**
      ...
      function onChartCallback(chart) {
        ...
        function canvasToSVG() {
          if (chart.boost && chart.boost.wgl && isChartSeriesBoosting(chart)) {
      ...
      class WGLShader {
        ...
        handleErrors() {
          if (this.errors.length) {
            error(
              "[highcharts boost] shader error - " + this.errors.join("\n")
            );
          }
        }
        /**
  ...
  _registerModule(
    _modules,
    "Extensions/Boost/WGLVertexBuffer.js",
    [],
    function () {
      /* *
       *
  ...
  _registerModule(
    ...
    [
      ...
      _modules["Core/Utilities.js"],
      _modules["Extensions/Boost/WGLDrawMode.js"],
      _modules["Extensions/Boost/WGLShader.js"],
      _modules["Extensions/Boost/WGLVertexBuffer.js"],
    ],
    function (Color, H, U, WGLDrawMode, WGLShader, WGLVertexBuffer) {
      /* *
       *
       *  Copyright (c) 2019-2021 Highsoft AS
       *
      ...
      class WGLRenderer {
        ...
        pushSeriesData(series, inst) {
          ...
          const vertice = (x, y, checkTreshold, pointSize = 1, color) => {
            ...
            if (
              pixelRatio !== 1 &&
              (!settings.useGPUTranslations || inst.skipTranslation)
            ) {
              x *= pixelRatio;
              y *= pixelRatio;
          ...
          this.series.forEach((s, si) => {
            ...
            if (s.colorData.length > 0) {
              ...
              gl.disableVertexAttribArray(
                gl.getAttribLocation(shader.getProgram(), "aColor")
              );
            }
            ...
            if (s.drawMode === "POINTS") {
              shader.setPointSize(
                pick(options.marker && options.marker.radius, 0.5) *
                  2 *
                  pixelRatio
              );
            }
            ...
            if (s.hasMarkers && showMarkers) {
              shader.setPointSize(
                pick(options.marker && options.marker.radius, 5) *
                  2 *
                  pixelRatio
              );
  ...
  _registerModule(
    ...
    [
      _modules["Extensions/Boost/BoostableMap.js"],
      _modules["Extensions/Boost/Boostables.js"],
      _modules["Extensions/Boost/BoostChart.js"],
      _modules["Core/Defaults.js"],
      _modules["Core/Globals.js"],
      _modules["Core/Utilities.js"],
      _modules["Extensions/Boost/WGLRenderer.js"],
    ],
    function (BoostableMap, Boostables, BoostChart, D, H, U, WGLRenderer) {
      /* *
       *
       *  Copyright (c) 2019-2021 Highsoft AS
       *
      ...
      function compose(SeriesClass, seriesTypes, wglMode) {
        ...
        if (wglMode) {
          ...
          if (AreaSeries && U.pushUnique(composedMembers, AreaSeries)) {
          ...
          if (BubbleSeries && U.pushUnique(composedMembers, BubbleSeries)) {
          ...
          if (ColumnSeries && U.pushUnique(composedMembers, ColumnSeries)) {
            extend(ColumnSeries.prototype, {
              fill: true,
              sampling: true,
            });
          }
          if (ScatterSeries && U.pushUnique(composedMembers, ScatterSeries)) {
      ...
      function createAndAttachRenderer(chart, series) {
        const ChartClass = chart.constructor,
        ...
        const boost = (target.boost = target.boost || {});
        ...
        if (!boost.target) {
          ...
          if (chart.renderer.forExport || !foSupported) {
            target.renderTarget = boost.target = chart.renderer
            ...
            boost.targetFo = chart.renderer
              ...
              .add(targetGroup);
            target.renderTarget = boost.target = doc.createElement("canvas");
      ...
      function onSeriesDestroy() {
        const series = this,
          chart = series.chart;
        if (chart.boost && chart.boost.markerGroup === series.markerGroup) {
      ...
      function getPoint(series, boostPoint) {
        ...
        point.category = pick(
          xAxis.categories ? xAxis.categories[point.x] : point.x, // @todo simplify
          point.x
        );
      ...
      function seriesRenderCanvas() {
        const options = this.options || {},
          ...
          threshold = options.threshold,
          isRange =
            this.pointArrayMap && this.pointArrayMap.join(",") === "low,high",
          isStacked = !!options.stacking,
        ...
        if (!isChartSeriesBoosting(chart)) {
          ...
          if (chart.boost && this.markerGroup === chart.boost.markerGroup) {
        ...
        function processPoint(d, i) {
          ...
          if (!chartDestroyed) {
            ...
            if (x >= xMin && x <= xMax && isYInside) {
              ...
              if (sampling) {
                if (typeof minI === "undefined" || clientX === lastClientX) {
                  if (!isRange) {
                    low = y;
                  }
      ...
      function wrapSeriesGetExtremes(proceed) {
        if (this.boosted && hasExtremes(this)) {
          return {};
        }
        return proceed.apply(this, [].slice.call(arguments, 1));
      }
  ...
  _registerModule(
    ...
    [
      _modules["Extensions/Boost/BoostChart.js"],
      _modules["Extensions/Boost/BoostSeries.js"],
          ...
          getContext: function () {
            ...
            if (!boost.canvas) {
              ...
              boost.target = chart.renderer
                ...
                .addClass("highcharts-boost-canvas")
                .add(targetGroup);
              ctx = boost.targetCtx = boost.canvas.getContext("2d");
              if (chart.inverted) {
            ...
            H.eachAsync(
              ...
              function (d, i) {
                ...
                if (!chartDestroyed) {
                  ...
                  if (i % CHUNK_SIZE === 0) {
                    if (series.boost && series.boost.copy) {
                      series.boost.copy();
                    } else if (series.chart.boost && series.chart.boost.copy) {
        ...
        Chart.prototype.callbacks.push(function (chart) {
          ...
          function clear() {
            ...
            if (boost.canvas) {
              boost.canvas
                .getContext("2d")
                .clearRect(0, 0, boost.canvas.width, boost.canvas.height);
            }
          }
  ...
  _registerModule(
    ...
    [
      ...
      _modules["Extensions/Boost/NamedColors.js"],
      _modules["Core/Utilities.js"],
    ],
    function (BoostChart, BoostSeries, H, initCanvasBoost, NamedColors, U) {
      /* *
       *
       *  Copyright (c) 2019-2021 Highsoft AS
       *
      ...
      function compose(ChartClass, SeriesClass, seriesTypes, ColorClass) {
        ...
        if (ColorClass && U.pushUnique(composedClasses, ColorClass)) {
          ColorClass.names = Object.assign(
            Object.assign({}, ColorClass.names),
  ...
  _registerModule(
    ...
    [_modules["Core/Globals.js"], _modules["Extensions/Boost/Boost.js"]],
    function (Highcharts, Boost) {
      const G = Highcharts;
