...
(function (factory) {
  if (typeof module === "object" && module.exports) {
    ...
    define("highcharts/modules/draggable-points", ["highcharts"], function (
      Highcharts
    ) {
      factory(Highcharts);
  ...
  _registerModule(
    ...
    function (A, Chart, H, Point, Series, SeriesRegistry, U) {
      /* *
       *
      ...
      const horizHandleFormatter = function (point) {
        const shapeArgs = point.shapeArgs || point.graphic.getBBox(),
      ...
      const columnDragDropProps = (seriesTypes.column.prototype.dragDropProps =
        {
          ...
          y: {
            ...
            beforeResize: function (guideBox, pointVals, point) {
              ...
              let height, diff;
              if (
                point.series.yAxis.reversed ? y < threshold : y >= threshold
              ) {
                ...
                guideBox.attr({
                  y: Math.round(
                    plotY + (plotThreshold ? plotThreshold - plotY : 0)
                  ),
                });
              }
            },
            ...
            handlePositioner: function (point) {
              const bBox =
                ...
                y = point.y || 0,
                bottom =
                  (!reversed && y >= threshold) || (reversed && y < threshold);
              return {
                x: bBox.x || 0,
      ...
      if (seriesTypes.xrange) {
        ...
        const xrangeHandlePositioner = function (point, xProp) {
            ...
            let newX = xAxis.toPixels(point[xProp], true),
      ...
      function isPointMovable(point) {
        ...
        return (
          ...
          ((series.yAxis && series.xAxis) || chart.mapView)
        );
      }
      /**
      ...
      function hasDraggedPastSensitivity(e, chart, sensitivity) {
        const orig = chart.dragDropData.origin,
          oldX = orig.chartX,
          oldY = orig.chartY,
          newX = e.chartX,
          newY = e.chartY,
      ...
      function updatePoints(chart, animation) {
        const newPoints = chart.dragDropData.newPoints,
      ...
      Point.prototype.getDropValues = function (origin, newPos, updateProps) {
        ...
        const limitToRange = function (val, direction) {
          const defaultPrecision = series[direction.toLowerCase() + "Axis"]
              ...
              : 0,
            precision = pick(
              options["dragPrecision" + direction],
              defaultPrecision
            ),
        ...
        const limitToMapRange = function (newPos, direction, key) {
          if (mapView) {
            const precision = pick(options["dragPrecision" + direction], 0),
            ...
            let min = pick(
                options["dragMin" + direction],
                lonLatMin && lonLatMin[key],
                -Infinity
              ),
              max = pick(
                options["dragMax" + direction],
                lonLatMax && lonLatMax[key],
                Infinity
              ),
              res = newPos[key];
        ...
        objectEach(updateProps, function (val, key) {
          const oldVal = pointOrigin.point[key],
        ...
        objectEach(series.dragDropProps, function (val, key) {
          ...
          if (
            ...
            handleAttrs.cursor =
              ...
              ((val.axis === "x") !== !!inverted ? "ew-resize" : "ns-resize");
      ...
      Chart.prototype.zoomOrPanKeyPressed = function (e) {
        ...
        const chartOptions = this.options.chart || {},
          ...
          zoomKey =
            chartOptions.zooming.key && chartOptions.zooming.key + "Key";
  ...
  _registerModule(
    _modules,
    "masters/modules/draggable-points.src.js",
    [],
    function () {}
  );
});
