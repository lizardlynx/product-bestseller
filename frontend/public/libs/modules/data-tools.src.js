...
(function (factory) {
  if (typeof module === "object" && module.exports) {
    ...
    define("highcharts/modules/data-tools", ["highcharts"], function (
      Highcharts
    ) {
      factory(Highcharts);
    ...
    function (U) {
      ...
      class DataTable {
        ...
        constructor(options = {}) {
          ...
          let rowCount = 0;
          for (
            let i = 0, iEnd = columnNames.length, column, columnName;
            i < iEnd;
            ++i
          ) {
        ...
        deleteRows(rowIndex, rowCount = 1, eventDetail) {
          ...
          if (rowCount > 0 && rowIndex < table.rowCount) {
            const columns = table.columns,
              columnNames = Object.keys(columns);
            for (
              let i = 0, iEnd = columnNames.length, column, deletedCells;
              i < iEnd;
              ++i
            ) {
        ...
        getCell(columnNameOrAlias, rowIndex) {
          const table = this;
          columnNameOrAlias =
            table.aliasMap[columnNameOrAlias] || columnNameOrAlias;
        ...
        getColumn(columnNameOrAlias, asReference) {
          return this.getColumns([columnNameOrAlias], asReference)[
            columnNameOrAlias
          ];
        }
        /**
        ...
        getColumnAsNumbers(columnNameOrAlias, useNaN) {
          ...
          if (column) {
            ...
            if (useNaN) {
              ...
              for (let i = 0; i < columnLength; ++i) {
                columnAsNumber.push(
                  table.getCellAsNumber(columnNameOrAlias, i)
                );
              }
            }
          }
        ...
        getColumns(columnNamesOrAliases, asReference) {
          ...
          for (
            let i = 0, iEnd = columnNamesOrAliases.length, column, columnName;
            i < iEnd;
            ++i
          ) {
        ...
        getRowObjects(
          ...
          for (
            let i = rowIndex,
              i2 = 0,
              iEnd = Math.min(table.rowCount, rowIndex + rowCount),
              column,
              row;
            i < iEnd;
            ++i, ++i2
        ...
        hasColumns(columnNamesOrAliases) {
          const table = this,
            ...
            columns = table.columns;
          for (
            let i = 0, iEnd = columnNamesOrAliases.length, columnName;
            i < iEnd;
            ++i
          ) {
            columnName = columnNamesOrAliases[i];
        ...
        setColumns(columns, rowIndex, eventDetail) {
          ...
          for (
            ...
            columnName = columnNames[i];
            column = columns[columnName];
            columnName = table.aliasMap[columnName] || columnName;
            if (reset) {
        ...
        setRows(rows, rowIndex = this.rowCount, eventDetail) {
          ...
          for (let i = 0, i2 = rowIndex, row; i < rowCount; ++i, ++i2) {
            ...
            if (row === DataTable.NULL) {
              ...
              for (
                let j = 0, jEnd = rowColumnNames.length, rowColumnName;
                j < jEnd;
                ++j
              ) {
  ...
  _registerModule(
    ...
    [_modules["Data/DataTable.js"], _modules["Core/Utilities.js"]],
    function (DataTable, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
      ...
      class DataConnector {
        ...
        getColumnOrder(usePresentationState) {
          ...
          if (names.length) {
            return names.sort(
              (a, b) => pick(columns[a].index, 0) - pick(columns[b].index, 0)
            );
          }
        }
        /**
        ...
        startPolling(refreshTime = 1000) {
          ...
          connector.polling = window.setTimeout(
            () =>
              connector
                .load()
                ["catch"]((error) =>
          ...
          if (options.parseDate) {
            ...
            if (!match) {
              ...
              if (
                ...
                result = match.getTime() - match.getTimezoneOffset() * 60000;
                // Timestamp
  ...
  _registerModule(_modules, "Data/DataCursor.js", [], function () {
    ...
    class DataCursor {
      ...
      emitCursor(table, cursor, event, lasting) {
        const tableId = table.id,
          state = cursor.state,
          listeners =
            this.listenerMap[tableId] && this.listenerMap[tableId][state];
        if (listeners) {
      ...
      remitCursor(tableId, cursor) {
        const cursors =
          this.stateMap[tableId] && this.stateMap[tableId][cursor.state];
        if (cursors) {
    ...
    (function (DataCursor) {
      ...
      function isEqual(cursorA, cursorB) {
        ...
        if (cursorA.type === "range" && cursorB.type === "range") {
          return (
            ...
            JSON.stringify(cursorA.columns) === JSON.stringify(cursorB.columns)
          );
        }
        return false;
      }
      ...
      function toPositions(cursor) {
        ...
        for (
          let row = cursor.firstRow, rowEnd = cursor.lastRow;
          row < rowEnd;
          ++row
        ) {
          if (!columns.length) {
          ...
          for (
            let column = 0, columnEnd = columns.length;
            column < columnEnd;
            ++column
          ) {
            positions.push({
      ...
      function toRange(cursor, defaultRange) {
        ...
        const range = {
          ...
          firstRow:
            (_b =
              (_a = cursor.row) !== null && _a !== void 0
                ? _a
                : defaultRange && defaultRange.firstRow) !== null &&
            _b !== void 0
              ? _b
              : 0,
          lastRow:
            (_d =
              (_c = cursor.row) !== null && _c !== void 0
                ? _c
                : defaultRange && defaultRange.lastRow) !== null &&
            _d !== void 0
              ? _d
              : Number.MAX_VALUE,
          state: cursor.state,
  ...
  _registerModule(
    ...
    "Data/DataPool.js",
    [
      _modules["Data/DataPoolDefaults.js"],
      _modules["Data/Connectors/DataConnector.js"],
    ],
    function (DataPoolDefaults, DataConnector) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
      ...
      class DataPool {
        ...
        getConnectorTable(name) {
          return this.getConnector(name).then((connector) => connector.table);
        }
        /**
  ...
  _registerModule(
    ...
    [
      _modules["Data/Converters/DataConverter.js"],
      _modules["Core/Utilities.js"],
    ],
    function (DataConverter, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
      ...
      class CSVConverter extends DataConverter {
        ...
        parse(options, eventDetail) {
          ...
          if (csv) {
            ...
            if (!itemDelimiter) {
              converter.guessedItemDelimiter = converter.guessDelimiter(lines);
            }
            ...
            for (rowIt = startRow; rowIt <= endRow; rowIt++) {
              if (lines[rowIt][0] === "#") {
                ...
                converter.parseCSVRow(lines[rowIt], rowIt - startRow - offset);
              }
            }
            if (
              dataTypes.length &&
        ...
        parseCSVRow(columnStr, rowNumber) {
          const converter = this,
            ...
            itemDelimiter =
              converter.options.itemDelimiter || converter.guessedItemDelimiter;
            ...
            for (let j = 0; j < columnStr.length; j++) {
              ...
              if (c === '"') {
                ...
                if (!isNaN(Date.parse(token))) {
                  potDelimiters[c]++;
                } else if (isNaN(Number(token)) || !isFinite(Number(token))) {
                  potDelimiters[c]++;
                }
                token = "";
              } else {
       ...
       * Default options
       */
      CSVConverter.defaultOptions = Object.assign(
        Object.assign({}, DataConverter.defaultOptions),
        { lineDelimiter: "\n" }
      );
      /* *
       *
  ...
  _registerModule(
    ...
    [
      _modules["Data/Converters/CSVConverter.js"],
      _modules["Data/Connectors/DataConnector.js"],
      _modules["Core/Utilities.js"],
    ],
    function (CSVConverter, DataConnector, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
  ...
  _registerModule(
    ...
    function (DataConverter, U) {
      ...
      class GoogleSheetsConverter extends DataConverter {
        ...
        parse(options, eventDetail) {
          const converter = this,
            ...
            columns = (
              (parseOptions.json && parseOptions.json.values) ||
              []
       ...
       * Default options
       */
      GoogleSheetsConverter.defaultOptions = Object.assign(
        {},
        DataConverter.defaultOptions
      );
      /* *
  ...
  _registerModule(
    ...
    [
      _modules["Data/Connectors/DataConnector.js"],
      _modules["Data/Converters/GoogleSheetsConverter.js"],
      _modules["Core/Utilities.js"],
    ],
    function (DataConnector, GoogleSheetsConverter, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
      ...
      (function (GoogleSheetsConnector) {
        ...
        function buildFetchURL(apiKey, sheetKey, options = {}) {
          return (
            ...
            (options.onlyColumnNames ? "A1:Z1" : buildQueryRange(options)) +
            "?alt=json" +
  ...
  _registerModule(
    ...
    function (DataConverter, U) {
        ...
        export(connector, options = this.options) {
          ...
          for (let columnIndex = 0; columnIndex < columnsCount; columnIndex++) {
            ...
            for (let rowIndex = 0; rowIndex < columnLength; rowIndex++) {
              ...
              if (columnIndex === columnsCount - 1) {
                htmlRows.push("<tr>" + rowArray[rowIndex].join("") + "</tr>");
              }
            }
          }
          let caption = "";
       ...
       * Default options
       */
      HTMLTableConverter.defaultOptions = Object.assign(
        Object.assign({}, DataConverter.defaultOptions),
        { useRowspanHeaders: true, useMultiLevelHeaders: true }
      );
      /* *
       *
       *  Default Export
  ...
  _registerModule(
    ...
    [
      ...
      _modules["Data/Converters/HTMLTableConverter.js"],
      _modules["Core/Utilities.js"],
    ],
    function (DataConnector, H, HTMLTableConverter, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
  ...
  _registerModule(
    ...
    [_modules["Data/Modifiers/DataModifier.js"], _modules["Core/Utilities.js"]],
    function (DataModifier, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
      ...
      class ChainModifier extends DataModifier {
        ...
        constructor(options, ...chain) {
          ...
          for (
            let i = 0, iEnd = optionsChain.length, ModifierClass;
            i < iEnd;
            ++i
          ) {
        ...
        modify(table, eventDetail) {
          ...
          for (let i = 0, iEnd = modifiers.length; i < iEnd; ++i) {
            ...
            promiseChain = promiseChain.then((chainTable) =>
              modifier.modify(chainTable.modified, eventDetail)
            );
          }
          promiseChain = promiseChain.then((chainTable) => {
    ...
    function (DataModifier, U) {
      ...
      class InvertModifier extends DataModifier {
        ...
        modifyRows(table, rows, rowIndex, eventDetail) {
          ...
          for (
            let i = 0, i2 = rowIndex, iEnd = rows.length, row;
            i < iEnd;
            ++i, ++i2
          ) {
            row = rows[i];
        ...
        modifyTable(table, eventDetail) {
          ...
          if (ranges.length) {
            const columns = table.getColumns(),
              ...
              modified = table.modified;
            for (
              let i = 0, iEnd = ranges.length, range, rangeColumn;
              i < iEnd;
              ++i
            ) {
              range = ranges[i];
              ...
              for (
                let j = 0, jEnd = rangeColumn.length, cell, row;
                j < jEnd;
                ++j
              ) {
                cell = rangeColumn[j];
  ...
  _registerModule(
    ...
    [
      _modules["Data/Modifiers/DataModifier.js"],
      _modules["Data/DataTable.js"],
      _modules["Core/Utilities.js"],
    ],
    function (DataModifier, DataTable, U) {
      /* *
       *
       *  (c) 2009-2023 Highsoft AS
       *
      ...
      class SortModifier extends DataModifier {
            ...
            if (orderInColumn) {
              ...
              table.modified.setColumn(
                ...
                modifier
                  .modifyTable(
                    new DataTable({
                      columns: table.getColumns([orderByColumn, orderInColumn]),
                    })
                  )
        ...
        modifyTable(table, eventDetail) {
          ...
          const columnNames = table.getColumnNames(),
            ...
            rowReferences = table.getRows().map((row, index) => ({
              index,
              row,
            })),
  ...
  _registerModule(
    ...
    [
      ...
      _modules["Data/Connectors/DataConnector.js"],
      _modules["Data/Converters/DataConverter.js"],
      _modules["Data/DataCursor.js"],
      _modules["Data/Modifiers/DataModifier.js"],
      _modules["Data/DataPool.js"],
      _modules["Data/DataTable.js"],
    ],
    function (
      Highcharts,
