...
(function (factory) {
  ...
  _registerModule(
    ...
    function (SeriesRegistry, U) {
      ...
      const {
        seriesTypes: {
          mapline: {
            prototype: { pointClass: MapLinePoint },
          },
        },
      } = SeriesRegistry;
      ...
      class FlowMapPoint extends MapLinePoint {
        ...
        isValid() {
          ...
          [this.options.to, this.options.from].forEach(function (toOrFrom) {
            valid = !!(
              valid &&
              toOrFrom &&
  ...
  _registerModule(
    ...
    function (SVGRenderer) {
      ...
      function bottomButton(x, y, w, h, options) {
        if (options) {
          const r =
            (options === null || options === void 0 ? void 0 : options.r) || 0;
          options.brBoxY = y - r;
  ...
  _registerModule(
    ...
    function (Chart, D, SVGRenderer, U) {
      /* *
       *
       *  (c) 2010-2021 Torstein Honsi
       *
  ...
  _registerModule(
    ...
    [
      _modules["Series/ColorMapComposition.js"],
      _modules["Maps/MapUtilities.js"],
      _modules["Core/Series/SeriesRegistry.js"],
      _modules["Core/Utilities.js"],
    ],
    function (ColorMapComposition, MapUtilities, SeriesRegistry, U) {
      /* *
       *
       *  (c) 2010-2021 Torstein Honsi
       *
      ...
      class MapPoint extends ScatterSeries.prototype.pointClass {
        ...
        applyOptions(options, x) {
          const series = this.series,
            point = super.applyOptions.call(this, options, x),
            joinBy = series.joinBy;
          if (series.mapData && series.mapMap) {
            const joinKey = joinBy[1],
              mapKey = super.getNestedProperty.call(point, joinKey),
          ...
          if (bounds) {
            ...
            if (mapView && isNumber(propMiddleLon) && isNumber(propMiddleLat)) {
              ...
              bounds.midX =
                bounds.x1 +
                (bounds.x2 - bounds.x1) *
                  pick(this.middleX, isNumber(propMiddleX) ? propMiddleX : 0.5);
        ...
        zoomTo(animOptions) {
          ...
          if (mapView && bounds) {
            const inset =
              isNumber(point.insetIndex) && mapView.insets[point.insetIndex];
            if (inset) {
  ...
  _registerModule(
    _modules,
    "Maps/MapViewInsetsOptionsDefault.js",
    [],
    function () {
      /* *
       *
  ...
  _registerModule(_modules, "Core/Geometry/PolygonClip.js", [], function () {
     ...
     *
     * */
    const isInside = (clipEdge1, clipEdge2, p) =>
    ...
    const intersection = (clipEdge1, clipEdge2, prevPoint, currentPoint) => {
      const dc = [clipEdge1[0] - clipEdge2[0], clipEdge1[1] - clipEdge2[1]],
        dp = [prevPoint[0] - currentPoint[0], prevPoint[1] - currentPoint[1]],
    ...
    (function (PolygonClip) {
      ...
      PolygonClip.clipLineString = (line, boundsPolygon) => {
        const ret = [],
          l = PolygonClip.clipPolygon(line, boundsPolygon, false);
          ...
          for (let i = 0; i < inputList.length; i++) {
            ...
            if (isInside(clipEdge1, clipEdge2, currentPoint)) {
              if (!isInside(clipEdge1, clipEdge2, prevPoint)) {
                outputList.push(
                  intersection(clipEdge1, clipEdge2, prevPoint, currentPoint)
                );
              }
  ...
  _registerModule(
    _modules,
    "Maps/Projections/LambertConformalConic.js",
    [],
    function () {
      /* *
       * Lambert Conformal Conic projection
       * */
      const sign = Math.sign || ((n) => (n === 0 ? 0 : n > 0 ? 1 : -1)),
        scale = 63.78137,
        deg2rad = Math.PI / 180,
        halfPI = Math.PI / 2,
        eps10 = 1e-6,
        tany = (y) => Math.tan((halfPI + y) / 2);
      class LambertConformalConic {
        constructor(options) {
          var _a;
          const parallels = (options.parallels || []).map((n) => n * deg2rad),
            lat1 = parallels[0] || 0,
          ...
          let n =
            lat1 === lat2
              ? Math.sin(lat1)
              : Math.log(cosLat1 / Math.cos(lat2)) /
    ...
    class Miller {
      ...
      forward(lonLat) {
        return [
          ...
          1.25 *
            scale *
            Math.log(Math.tan(quarterPI + 0.4 * lonLat[1] * deg2rad)),
        ];
      }
      inverse(xy) {
        return [
  ...
  _registerModule(
    _modules,
    "Maps/Projections/Orthographic.js",
    [],
    function () {
      /* *
       * Orthographic projection
  ...
  _registerModule(
    ...
    [
      _modules["Maps/Projections/LambertConformalConic.js"],
      _modules["Maps/Projections/EqualEarth.js"],
      _modules["Maps/Projections/Miller.js"],
      _modules["Maps/Projections/Orthographic.js"],
      _modules["Maps/Projections/WebMercator.js"],
    ],
    function (
  ...
  _registerModule(
    ...
    "Maps/Projection.js",
    [
      _modules["Core/Geometry/PolygonClip.js"],
      _modules["Maps/Projections/ProjectionRegistry.js"],
      _modules["Core/Utilities.js"],
    ],
    function (PC, registry, U) {
      /* *
       *
      ...
      class Projection {
        ...
        constructor(options = {}) {
          ...
          const ProjectionDefinition = name
            ? Projection.registry[name]
            : void 0;
          ...
          this.bounds =
            projectedBounds === "world" ? def && def.bounds : projectedBounds;
        }
          ...
          const getIntersect = (line, dim, val) => {
            ...
            if (typeof val === "number" && p1[dim] >= val !== p2[dim] >= val) {
              const fraction = (val - p1[dim]) / (p2[dim] - p1[dim]),
                crossingVal =
                  p1[otherDim] + fraction * (p2[otherDim] - p1[otherDim]);
            ...
            if (isPolygon) {
              ...
              while (i >= 0) {
                ...
                const lonPlus = wrapLon(
                  antimeridian + intersections[i].direction * floatCorrection
                );
              ...
              if (polarIntersection) {
                for (let i = 0; i < polygons.length; i++) {
                  ...
                  if (indexOf > -1) {
                    const polarLatitude = (lat < 0 ? -1 : 1) * this.maxLatitude;
                    const lon1 = wrapLon(
                      antimeridian + direction * floatCorrection
                    );
                    const lon2 = wrapLon(
                      antimeridian - direction * floatCorrection
                    );
                ...
                if (someOutside) {
                  ...
                  if (boundsPolygon) {
                    ...
                    if (isPolygon) {
                      ...
                      clipLineString(points, boundsPolygon).forEach(
                        (points) => {
                          movedTo = false;
  ...
  _registerModule(
    ...
    "Maps/MapView.js",
    [
      _modules["Maps/MapViewOptionsDefault.js"],
      _modules["Maps/MapViewInsetsOptionsDefault.js"],
      _modules["Extensions/GeoJSON.js"],
      _modules["Core/Chart/MapChart.js"],
      _modules["Maps/MapUtilities.js"],
      _modules["Maps/Projection.js"],
      _modules["Core/Utilities.js"],
    ],
    function (
      defaultOptions,
      defaultInsetsOptions,
      GeoJSONModule,
      MapChart,
      MU,
      Projection,
      ...
      class MapView {
        ...
        static mergeInsets(a, b) {
          ...
          const insetsObj = merge(toObject(a), toObject(b)),
            insets = Object.keys(insetsObj).map((key) => insetsObj[key]);
          return insets;
        }
        ...
        constructor(chart, options) {
          ...
          if (!(this instanceof MapViewInset)) {
            ...
            const geoBounds =
              allGeoBounds.length && MapView.compositeBounds(allGeoBounds);
          ...
          this.eventsToUnbind.push(
            addEvent(chart, "afterSetChartSize", () => {
        ...
        getMapBBox() {
          ...
          if (bounds) {
            const padding = this.padding,
              ...
              width = (bounds.x2 - bounds.x1) * scale + padding[1] + padding[3],
              height =
                (bounds.y2 - bounds.y1) * scale + padding[0] + padding[2];
            return {
              width,
              height,
              x: p1.x - padding[3],
        ...
        getProjectedBounds() {
          ...
          const allBounds = this.chart.series.reduce((acc, s) => {
            ...
            if (bounds && s.options.affectsMapView !== false) {
              acc.push(bounds);
            }
            return acc;
          }, []);
          ...
          if (fitToGeometry) {
            if (!this.fitToGeometryCache) {
              if (fitToGeometry.type === "MultiPoint") {
                const positions = fitToGeometry.coordinates.map((lonLat) =>
                    projection.forward(lonLat)
                  ),
                  xs = positions.map((pos) => pos[0]),
                  ys = positions.map((pos) => pos[1]);
        ...
        projectedUnitsToLonLat(point) {
          ...
          for (const inset of this.insets) {
            if (
              ...
              const insetProjectedPoint = inset.pixelsToProjectedUnits(pxPoint),
                coordinates = inset.projection.inverse([
                  insetProjectedPoint.x,
          ...
          const onPan = (e) => {
            ...
            if (
              ...
              const key = `${mouseDownX},${mouseDownY}`,
                { chartX, chartY } = e.originalEvent;
              // Reset starting position
              if (key !== mouseDownKey) {
                mouseDownKey = key;
                mouseDownCenterProjected = this.projection.forward(this.center);
                mouseDownRotation = (
              ...
              if (
                ...
                if (mouseDownRotation) {
                  const lon =
                      (mouseDownX - chartX) * ratio - mouseDownRotation[0],
                    lat = clamp(
      ...
      MapView.compositeBounds = (arrayOfBounds) => {
        if (arrayOfBounds.length) {
          return arrayOfBounds.slice(1).reduce((acc, cur) => {
        ...
        getField(padded = true) {
          ...
          if (hitZone) {
            const padding = padded ? this.padding : [0, 0, 0, 0],
              ...
              xs = polygon.map((xy) => xy[0]),
              ys = polygon.map((xy) => xy[1]),
        ...
        render() {
          ...
          if (borderPath && mapView.group) {
            ...
            const d = (borderPath.coordinates || []).reduce(
              (d, lineString) =>
                lineString.reduce((d, point, i) => {
                  let [x, y] = point;
                  if (options.units === "percent") {
                    x =
                      chart.plotLeft +
                      relativeLength(`${x}%`, field.width, field.x);
                    y =
                      chart.plotTop +
                      relativeLength(`${y}%`, field.height, field.y);
                  }
  ...
  _registerModule(
    ...
    [
      ...
      _modules["Series/ColorMapComposition.js"],
      _modules["Series/CenteredUtilities.js"],
      _modules["Core/Globals.js"],
      _modules["Core/Legend/LegendSymbol.js"],
      _modules["Core/Chart/MapChart.js"],
      _modules["Series/Map/MapPoint.js"],
      _modules["Maps/MapView.js"],
    ...
    function (
      ...
      class MapSeries extends ScatterSeries {
        ...
        drawPoints() {
          ...
          transformGroups.forEach((transformGroup, i) => {
            ...
            if (
              ...
              const step = (now, fx) => {
                const scaleStep = startScale + (scale - startScale) * fx.pos;
                transformGroup.attr({
                  translateX:
                    startTranslateX +
              ...
              transformGroup.attr(
                merge(svgTransform, { "stroke-width": strokeWidth / scale })
              );
          ...
          if (!this.bounds && this.chart.mapView) {
            ...
            (this.points || []).forEach(function (point) {
              if (point.path || point.geometry) {
                ...
                } else if (isArray(point.path) && point.path[0] === "M") {
        ...
        getStrokeWidth(options) {
          ...
          return options[
            (pointAttrToOptions && pointAttrToOptions["stroke-width"]) ||
              "borderWidth"
          ];
        }
        /**
        ...
        pointAttribs(point, state) {
          ...
          if (state) {
            ...
            attr.stroke =
              (_a = stateOptions.borderColor) !== null && _a !== void 0
                ? _a
                : point.color;
          }
          ...
          if (attr.dashstyle && mapView && isNumber(seriesStrokeWidth)) {
          ...
          attr["stroke-linecap"] = attr["stroke-linejoin"] =
        ...
        processData() {
          ...
          let mapView = this.chart.mapView,
            mapDataObject =
              ...
              (isObject(options.mapData, true)
                ? mapView.getGeoMap(options.mapData)
                : mapView.geoMap),
          ...
          if (isArray(options.mapData)) {
            ...
            mapDataObject &&
            mapDataObject.type === "FeatureCollection"
          ) {
            ...
            data.forEach(function (val, i) {
              ...
              if (isNumber(val)) {
                ...
                for (let j = 0; j < pointArrayMap.length; ++j, ++ix) {
                  if (pointArrayMap[j] && typeof val[ix] !== "undefined") {
          ...
          if (mapView) {
            ...
            series.points.forEach(function (point) {
              ...
              if (
                ...
                point.plotX =
                  point.bounds.midX * svgTransform.scaleX +
                  svgTransform.translateX;
                point.plotY =
                  point.bounds.midY * svgTransform.scaleY +
  ...
  _registerModule(
    ...
    [
      _modules["Series/FlowMap/FlowMapPoint.js"],
      _modules["Series/Map/MapSeries.js"],
      _modules["Core/Series/SeriesRegistry.js"],
      _modules["Core/Utilities.js"],
    ],
    function (FlowMapPoint, MapSeries, SeriesRegistry, U) {
      /* *
       *
       *  (c) 2010-2022 Askel Eirik Johansson, Piotr Madej
       *
      ...
      class FlowMapSeries extends MapLineSeries {
        ...
        static markerEndPath(lCorner, rCorner, topCorner, options) {
          const width = relativeLength(
            options.width || 0,
            this.getLength(rCorner[0] - lCorner[0], rCorner[1] - lCorner[1])
          );
        ...
        getLinkWidth(point) {
          ...
          return (
            ((weight - smallestWeight) * (maxWidthLimit - minWidthLimit)) /
              (greatestWeight - smallestWeight || 1) +
            minWidthLimit
          );
        }
        /**
         ...
         */
        pointAttribs(point, state) {
          const attrs = MapSeries.prototype.pointAttribs.call(
            this,
            point,
            state
          );
        ...
        getPointShapeArgs(point) {
          ...
          let toX = toPos.x || 0,
            ...
            offset =
              (markerEndOptions &&
                ...
                markerEndOptions.height) ||
              0;
          if (!defined(curveFactor)) {
            // Automate the curveFactor value.
          ...
          if (offset) {
            ...
            let [offsetX, offsetY] = FlowMapSeries.normalize(
              arcPointX - toX,
              arcPointY - toY
            );
            offsetX *= offset;
          ...
          let [fromXToArc, fromYToArc] = FlowMapSeries.normalize(
            arcPointX - fromX,
            arcPointY - fromY
          );
          tmp = fromXToArc;
          ...
          let [toXToArc, toYToArc] = FlowMapSeries.normalize(
            arcPointX - toX,
            arcPointY - toY
          );
          tmp = toXToArc;
          ...
          const shapeArgs = {
            d: [
              ["M", fromX - fromXToArc, fromY - fromYToArc],
              [
                "Q",
                arcPointX - wX,
                arcPointY - wY,
          ...
          if (markerEndOptions && markerEndOptions.enabled && shapeArgs.d) {
            const marker = FlowMapSeries.markerEndPath(
          ...
          const fromPoint = point.options.from,
  ...
  _registerModule(
    _modules,
    "masters/modules/flowmap.src.js",
    [],
    function () {}
  );
});
