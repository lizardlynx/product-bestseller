...
(function (factory) {
  if (typeof module === "object" && module.exports) {
    ...
    define("highcharts/modules/arc-diagram", [
      "highcharts",
  ...
  _registerModule(
    ...
    [
      _modules["Series/NodesComposition.js"],
      _modules["Core/Series/SeriesRegistry.js"],
      _modules["Core/Utilities.js"],
    ],
    function (NodesComposition, SeriesRegistry, U) {
      /* *
       *
       *  Arc diagram module
       *
  ...
  _registerModule(
    ...
    [
      _modules["Series/ArcDiagram/ArcDiagramPoint.js"],
      _modules["Series/Sankey/SankeyColumnComposition.js"],
    ...
    function (
      ...
      const {
        seriesTypes: { column: ColumnSeries, sankey: SankeySeries },
      } = SeriesRegistry;
      ...
      class ArcDiagramSeries extends SankeySeries {
        ...
        createNodeColumns() {
          ...
          column.sankeyColumn.getTranslationFactor = (series) => {
            ...
            let skipPoint,
              ...
              remainingWidth =
                (chart.plotSizeX || 0) -
                ((series.options.marker && series.options.marker.lineWidth) ||
                  0) -
          ...
          column.sankeyColumn.offset = function (node, factor) {
            ...
            let offset = column.sankeyColumn.additionalSpace || 0,
              ...
              maxRadius = Math.min(
                chart.plotWidth,
                chart.plotHeight,
                (column.sankeyColumn.maxLength || 0) / series.nodes.length -
                  nodePadding
              );
            for (let i = 0; i < column.length; i++) {
              const sum = column[i].getSum() * (column.sankeyColumn.scale || 0);
        ...
        translateLink(point) {
          const series = this,
            ...
            linkWeight = pick(
              ...
              Math.max(
                (point.weight || 0) * translationFactor * fromNode.scale,
          ...
          const getX = (node, fromOrTo) => {
            const linkLeft =
              (node.offset(point, fromOrTo) || 0) * translationFactor;
            const x = Math.min(
              node.nodeX + linkLeft,
          ...
          let fromX = centeredLinks
            ...
            toX = centeredLinks
              ? toNode.nodeX + ((toNode.shapeArgs.height || 0) - linkWeight) / 2
              : getX(toNode, "linksTo"),
            bottom = nodeTop,
          ...
          point.linkBase = [fromX, fromX + linkWeight, toX, toX + linkWeight];
          const linkRadius =
          ...
          point.tooltipPos = chart.inverted
            ...
            : [point.dlBox.x, point.dlBox.y + linkWeight / 2];
        ...
        translateNode(node, column) {
          const series = this,
            ...
            crisp =
              (Math.round((options.marker && options.marker.lineWidth) || 0) %
                2) /
              2,
            ...
            top =
              (parseInt(options.offset, 10) *
                ((chart.inverted ? chart.plotWidth : chart.plotHeight) -
                  (Math.floor(
                    ...
                    ((column.sankeyColumn.scale || 0) *
                      (column.sankeyColumn.maxRadius || 0)) /
                      2))) /
              100;
          node.sum = sum;
        ...
        pointAttribs(point, state) {
          if (point && point.isNode) {
            const _a = Series.prototype.pointAttribs.apply(this, arguments),
              { opacity } = _a,
              attrs = __rest(_a, ["opacity"]);
            return attrs;
          }
  ...
  _registerModule(
    _modules,
    "masters/modules/arc-diagram.src.js",
    [],
    function () {}
  );
});
