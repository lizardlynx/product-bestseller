...
(function (factory) {
  ...
  _registerModule(
    ...
    [
      ...
      _modules["Extensions/ExportData/ExportDataDefaults.js"],
      _modules["Core/Globals.js"],
      _modules["Core/Defaults.js"],
      _modules["Extensions/DownloadURL.js"],
      _modules["Core/Series/SeriesRegistry.js"],
      _modules["Core/Utilities.js"],
    ],
    function (AST, ExportDataDefaults, H, D, DownloadURL, SeriesRegistry, U) {
      /* *
       *
       *  Experimental data export module for Highcharts
       *
      ...
      function chartGetDataRows(multiLevelHeaders) {
        const hasParallelCoords = this.hasParallelCoordinates,
          ...
          getCategoryAndDateTimeMap = function (series, pointArrayMap, pIdx) {
            ...
            pointArrayMap.forEach(function (prop) {
              const axisName =
                  ((series.keyToAxis && series.keyToAxis[prop]) || prop) +
                  "Axis",
          ...
          getPointArray = function (series, xAxis) {
            const namedPoints = series.data.filter(
            ...
            if (
              ...
              if (series.pointArrayMap) {
                const pointArrayMapCheck = series.pointArrayMap.filter(
                  (p) => p === "x"
                );
        ...
        this.series.forEach(function (series) {
          ...
          if (
            ...
            if (
              !find(xAxisIndices, function (index) {
            ...
            series.options.data.forEach(function eachData(options, pIdx) {
              ...
              series.pointClass.prototype.applyOptions.apply(mockPoint, [
                options,
              ]);
              key = mockPoint.x;
              if (
                ...
                const keysFromActualSeries = Object.keys(rows).filter(
                    (i) =>
                      rows[i].seriesIndices.includes(mockSeries.index) && key
                  ),
                  // find all properties, which start with actual key
                  existingKeys = keysFromActualSeries.filter(
                    (propertyName) => propertyName.indexOf(String(key)) === 0
                  );
        ...
        if (options.exporting.tableCaption !== false) {
          treeChildren.push({
            ...
            textContent: pick(
              ...
              options.title.text ? options.title.text : "Chart"
            ),
          });
        }
        // Find longest row
  ...
  _registerModule(
    ...
    [
      _modules["Core/Globals.js"],
      _modules["Extensions/ExportData/ExportData.js"],
    ],
    function (Highcharts, ExportData) {
      const G = Highcharts;
