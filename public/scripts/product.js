...
function createProductHTML(product) {
  ...
  let html = '';
  html = `<img class="block1" src="${product.image}" width="400px" alt="${product.title}"><div class="block2">
  ...
  for (const shop of Object.keys(product.prices)) {
    ...
    for (const price of prices) {
      html += `<div class="price ${price.comment == 'oldPrice'? 'old-price': ''}">${price.comment == 'priceOpt'?'Оптова ціна:':''}<span class="price">${price.price}</span></div>`;
    }
    html += '</div>';
  }

  html += '</div><div class="block3">';
  ...
  for (const shop of Object.keys(product.features)) {
    ...
    for (const feature of features) {
      ...
      else if (feature.title == 'url_key') originalLinks[shop] = `${dbShopsData[shop].product_url}${feature.value}`;
      else html += `<div class="feature"><div class="feature-description">${feature.title}</div><div class="price">${feature.value}</div></div>`;
    }
  }
  html += '</div>';
...
async function loadProduct(productId) {
  ...
  const res = await fetch('/item?' + new URLSearchParams('product=' + productId), {
    method: 'GET'
  }); 
  const resJSON = await res.json();
...
async function loadPrices(productId) {
  const res = await fetch('/prices?' + new URLSearchParams('product=' + productId), {
    method: 'GET'
  }); 
  const resJSON = await res.json();
  for (const shopId in resJSON) {
    ...
    buildChart(shop + '-container', 'Зміна ціни', shop, shopUrl, 'Ціна, грн', 'травень-червень, 2023', 4, resJSON[shopId]);
