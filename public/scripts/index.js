...
function addSavedCategory(savedElements, id, categoryChildren) {
  ...
  for (let child of currChildren) {
    ...
    if (childId in savedElements) {
      const childChildren = child.getElementsByClassName('category-children')[0];
...
function createCategoryHTML(category) {
  ...
  categoryDiv.innerHTML = `<div class="category-title" data-link="/category/${category.id}">${category.title}</div>
...
async function loadShopData(shopName) {
  const res = await fetch('/load'+shopName, {
    method: 'GET'
  }); 
...
async function loadCategories() {
  ...
  for (let category of categories) {
    ...
    const categoryChildren = categoryEl.getElementsByClassName('category-children')[0];
    const categoryTitle = categoryEl.getElementsByClassName('category-title')[0];
    ...
    if (parentId == null) {
      ...
      const parentChildren = parentEl[0].getElementsByClassName('category-children')[0];
    ...
    categoryTitle.addEventListener('click', () => getCategoryProducts(category.id));
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loadCategories();
  document.getElementById('auchan').addEventListener('click', () => loadShopData('Auchan'));
  document.getElementById('silpo').addEventListener('click', () => loadShopData('Silpo'));  
  document.getElementById('both').addEventListener('click', () => loadShopData('Both'));  
