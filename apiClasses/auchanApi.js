...
class AuchanApi extends BaseApi {
  ...
  async loadProductsByCategory(id) {
    ...
    console.log(this.#getProductByCategoryVariables.currentPage, this.#getProductByCategoryTotalPages);
    const res = await this.#processAuchanRequest(gqlQueries.getProductsByCategory, this.#getProductByCategoryVariables);
    if (res.error) return res;
    await delay(3000);
    this.#getProductByCategoryTotalPages = res.data.search.page_info.total_pages;
    this.#getProductByCategoryVariables.currentPage++;
    if (this.#getProductByCategoryVariables.currentPage > this.#getProductByCategoryTotalPages) {
  ...
  async loadCategories() {
    ...
    return await this.#processAuchanRequest(gqlQueries.getCategories, variables);  
  }
